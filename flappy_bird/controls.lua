...
function handleKeypressed(key)
    ...
    if state == "shop" then
        ...
        elseif string.match(key, "^%d$") then
            local shortcutIndex = tonumber(key)
            local index = getShopShortcutTarget(shopSection, shortcutIndex)
...
function handleMousepressed(x, y, button)
    if state == "menu" and button == 1 then
        for i = 1, #menuButtonBounds do
            local bounds = menuButtonBounds[i]
    ...
    if state == "shop" and button == 1 then
        if shopArrowBounds.left ~= nil then
            local bounds = shopArrowBounds.left
        ...
        if shopArrowBounds.right ~= nil then
            local bounds = shopArrowBounds.right
...
function updateCoinAnimation(dt)
    ...
    local bronzeData = getCoinDrawData("bronze")

    if bronzeData ~= nil and bronzeData.frames ~= nil and #bronzeData.frames > 1 then
        ...
        if coinSpriteFrameTimer >= coinSpriteFrameDuration then
            ...
            coinSpriteFrameIndex = (coinSpriteFrameIndex % #bronzeData.frames) + 1
        end
    end
end
